...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*uiSender*/, uint32 /* uiAction */)
    {
        ...
        if (BuffByLevel == true)
        {
            ...
            {
                player->CastSpell(player, vecBuffs[0], true); // Prayer of Fortitude (14767)(Improved)
                player->CastSpell(player, vecBuffs[1], true); // Greater Blessing of Kings (43223)
                player->CastSpell(player, vecBuffs[2], true); // Mark of the Wild (48469)(Improved)
                player->CastSpell(player, vecBuffs[3], true); // Prayer of Spirit (48074)
                player->CastSpell(player, vecBuffs[4], true); // Prayer of Shadow Protection (48170)
                player->CastSpell(player, vecBuffs[5], true); // Arcane Intellect (42995)
				player->CastSpell(player, vecBuffs[6], true); // Horn of Winter(57623)
				player->CastSpell(player, vecBuffs[7], true); // Blessing of Wisdom(20245)(Improved)
				player->CastSpell(player, vecBuffs[8], true); // Blessing of Sanctuary(20911)
				player->CastSpell(player, vecBuffs[9], true); // Blessing of Might(20045)(Improved)
            } // LEVEL 80
        }
        else
        {
