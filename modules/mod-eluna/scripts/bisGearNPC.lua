...
if not COGGO then
WorldDBExecute("TRUNCATE TABLE acore_world.bis_list")
end
end
...
local class = player:GetClass()
  ...
  local query = WorldDBQuery(string.format("SELECT * FROM acore_world.bis_list WHERE CharID=%i", getPlayerCharacterGUID(player)))
  ...
  player:GossipMenuAddItem(0, "Only GM Rank 3 can access this NPC.", 0, 0)
	...
	player:LearnSpell(1579)
        ...
        for _,v in ipairs(T[intid]) do
			...
			if COGGO then
			WorldDBExecute(string.format("INSERT INTO acore_world.bis_list VALUES (%i)", getPlayerCharacterGUID(player)))
	end
        end
